...
void bbs_sign(unsigned char *sig, int hashlen, void *hash, bbs_group_public_key_ptr gpk, bbs_group_private_key_ptr gsk)
{
  ...
  */
  ...
  unsigned char *hash_input = (unsigned char *)malloc(hash_input_length);
    ...
    int pri_key_length, char* pri_key)
{
    ...
    deserialize_public_key(&s_gpk[0], (unsigned char *)pub_key, pub_key_length);

    bbs_group_private_key_t s_gsk;
    s_gsk[0].param = sp;
    deserialize_private_key(&s_gsk[0], (unsigned char *)pri_key, pri_key_length);
