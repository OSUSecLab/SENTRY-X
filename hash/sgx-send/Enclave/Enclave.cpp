...
//struct keypair_list_s
//{
//    int payload;
//    void *pbkey;
//    void *privatekey;
//};
//std::vector<keypair_list_s> keypair_list;
void* curr_priv_key;
uint8_t pubmod[1000];
uint8_t pube[100];
...
sgx_status_t send_symkey(char *MessageIn, size_t MessageInLen, char *encMessageOut, size_t encMessageOutLen)
{
    ...
    void *pub_key;
    //printf("%d %d\n", message->pube[0], message->pube[1]);
    sgx_status_t ret = sgx_create_rsa_pub1_key(SGX_RSA3072_KEY_SIZE, SGX_RSA3072_PUB_EXP_SIZE,
    ...
    ret = sgx_rsa_pub_encrypt_sha256((void *)pub_key, (unsigned char*)pout, &pout_size, (unsigned char *)temp_key, 16); // 16 size of key

    unsigned char sig_temp_1[300] = {0, };
    unsigned char sig_temp_2[60] = {0, };
    memcpy(sig_temp_1, (unsigned long *)sig, 300);
    memcpy(sig_temp_2, (unsigned long *)(sig+300), 60);

    unsigned char temp_1[384] = {0, };
    unsigned char temp_2[384] = {0, };
    size_t enc_msg_size = sizeof(temp_1);

    ret = sgx_rsa_pub_encrypt_sha256((void *)pub_key, temp_1, &enc_msg_size, (unsigned char *)sig_temp_1, 300);
    ret = sgx_rsa_pub_encrypt_sha256((void *)pub_key, temp_2, &enc_msg_size, (unsigned char *)sig_temp_2, 60);
    ...
    memcpy(sendmessage.sig_1, temp_1, 384);
    memcpy(sendmessage.sig_2, temp_2, 384);
...
sgx_status_t send_infomessage(char *encMessageOut, size_t encMessageLen)
{
    ...
    std::copy(std::begin(temp_key), std::end(temp_key), std::begin(hash_val.symkey));
    hash_val.rand = message.rand;

    sgx_status_t status = sgx_sha256_msg((const uint8_t *) &hash_val, sizeof(hash_val), (sgx_sha256_hash_t *)message.hash_rand);
