...
std::vector<symkey_store_s> symkey_list;
struct pub_key_s
{
    uint8_t pubmod[1000];
    uint8_t pube[100];
...
sgx_status_t manage_rsa(void)
{
    ...
    memcpy(pubmod, p_key.mod, SGX_RSA3072_KEY_SIZE);
    memcpy(pube, p_key.e, SGX_RSA3072_PUB_EXP_SIZE);
    if (ret != SGX_SUCCESS)
        printf("key gen fail\n");
    ...
    //symkey_list.reserve(SYM_SIZE);
    //for (int i=0; i<SYM_SIZE; i++)
    symkey_list.reserve(0);
    for (int i=0; i<0; i++)
    {
        //symkey_list[i] = temp_key;
        symkey_list.push_back(temp_key);
    }

    EpidStatus result = kEpidErr;
    memcpy(sig_pout, pube, SGX_RSA3072_PUB_EXP_SIZE);
...
sgx_status_t send_pbkey(char *encMessageOut, size_t encMessageOutLen)
{
    ...
    memcpy(outmessage.pubmod, pubmod, SGX_RSA3072_KEY_SIZE);
...
sgx_status_t receive_symkey(char *encMessageIn, size_t encMessageLen)
{
    ...
    unsigned char sig_1[384], sig_2[384], enc_sig_1[384], enc_sig_2[384];
    uint8_t pout[384] = {0, };
    size_t pout_size = 384;
    memcpy(enc_sig_1, rcvdmessage->sig_1, 384);
    memcpy(enc_sig_2, rcvdmessage->sig_2, 384);

    sgx_status_t ret = sgx_rsa_priv_decrypt_sha256(curr_priv_key, (unsigned char*)sig_1, &pout_size, (unsigned char *)enc_sig_1, 384);

    pout_size = 384;
    ret = sgx_rsa_priv_decrypt_sha256(curr_priv_key, (unsigned char *)sig_2, &pout_size, (unsigned char *)enc_sig_2, 384);
    ...
    {
        pout_size = 384;
        ret = sgx_rsa_priv_decrypt_sha256(curr_priv_key, (unsigned char*)pout, &pout_size, (unsigned char *)encrypt, 384);
    ...
    symkey_list.push_back(symkey);

    memcpy(sig, sig_1, 300);
    memcpy((unsigned long *)(sig+300), sig_2, 60);
    ...
    sig_sym_s sig_sym;
    memcpy(sig_sym.sym, symkey.key, 16);
    memcpy(sig_sym.sig, rcvdmessage->sig_1, 16);
    sig_sym_list.push_back(sig_sym);
    free(decrypt);
    return SGX_SUCCESS;
}
...
sgx_status_t rcvd_infomessage(char *messageIn, size_t MessageInLen)
{
    ...
    struct hash_s hash_val;
    sgx_status_t status;

    hash_val.rand = message->rand;
    uint8_t hash_result[32];
    int idx=0;
    for (symkey_store_s key : symkey_list)
    {
        std::copy(std::begin(key.key), std::end(key.key), std::begin(hash_val.symkey));
        sgx_sha256_msg((const uint8_t *) &hash_val, sizeof(hash_val), (sgx_sha256_hash_t *)&hash_result);
        int flag = 0;
        for (int i=0; i<32; i++)
        {
            if (hash_result[i] != message->hash_rand[i])
            {
                flag = 1;
                break;
            }
        ...
        if (flag == 0)
        {
            ...
            printf("%d Decrypted %s", idx, temp->bsm);
        }
        idx++;
    }
