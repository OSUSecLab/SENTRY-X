...
void* send_bsm(void *arg)
{
    ...
    temp.tag = info.id;
    char* text = "I am here";
    memcpy (temp.msg, text, 20);

    AES_KEY enc_key;
    unsigned char enc_out[80];
    AES_set_encrypt_key((const unsigned char *)info.key, 128, &enc_key);
    AES_encrypt((const unsigned char *)&temp, enc_out, &enc_key);

    struct bsm_msg msg;
    const size_t seed = 0;
    msg.ts = 123;
    msg.tag = info.id;
    memcpy(&msg.encmsg, enc_out, 80);
...
void* recv_bsm(void *arg)
{
    int id = *(int *)arg;
    AES_KEY dec_key;
    unsigned char dec_out[80], dec_tag[4];
    const size_t seed = 0;
    int record_flag = 0;
    ...
    while (1)
    {
        if (bsm[id].size() > 0)
        {
            ...
            auto it = sym_list[id].find(tag);

            if (it == sym_list[id].end())
                continue;

            memcpy(key, &it->second.key, 32);

            auto start = chrono::high_resolution_clock::now();


            AES_set_decrypt_key((const unsigned char *)key, 128,&dec_key);
            AES_decrypt((const unsigned char*)&msg.encmsg, dec_out, &dec_key);
            struct enc_msg *output = (enc_msg *)dec_out;

            if (output->tag != tag)
            {
                printf("tag errors %d %d\n", output->tag, tag);
                continue;
            }

            printf("%d %s\n", output->tag, output->msg);
        ...
        if (queue[id].size() > 0)
        {
            ...
            if (msg_type == 1)
            {
                ...
                if (result) {
                    ...
                    int ottag = pub_msg.tag;


                    auto it = pub_list.find(ottag);
                    ...
                    pub_list.insert({ottag, received_key});
                    ...
                    char comb[36];
                    memcpy(comb, key, 32);
                    memcpy(comb+32, &tag, 4);
                    encrypt_len = RSA_public_encrypt(36, (unsigned char*)comb, (unsigned char*)encrypt, rsa, RSA_PKCS1_OAEP_PADDING);
            ...
            else if(msg_type == 2)
            {
                ...
                if (RSA_private_decrypt(received_sym.sym_len, (unsigned char*)received_sym.symkey, (unsigned char*)decrypt, priv_rsa, RSA_PKCS1_OAEP_PADDING) != -1)
                {
                    ...
                    sym_list[id].insert({tag, received_key});
