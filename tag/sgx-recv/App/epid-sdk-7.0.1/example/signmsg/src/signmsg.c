...
SigRl* sign_sig_rl = NULL;
GroupPubKey sign_pub_key = {0};
PrivKey priv_key = {0};
MembershipCredential member_credential = {0};
size_t sign_sig_rl_size = 0;
MemberParams params = {0};
size_t member_size = 0;

void sign_init(void const* basename,
  ...
  do {
    ...
    if (privkey_size == sizeof(PrivKey)) {
      ...
      sts = EpidDecompressPrivKey(&sign_pub_key, (CompressedPrivKey*)priv_key_ptr,
                                  &priv_key);
      if (kEpidNoErr != sts) {
        break;
      }
    ...
    if (privkey_size == sizeof(PrivKey) ||
      ...
      sts = EpidProvisionKey(member, &sign_pub_key, &priv_key, member_precomp);
      if (kEpidNoErr != sts) {
        break;
      }
    } else if (privkey_size == sizeof(MembershipCredential)) {
      sts = EpidProvisionCredential(member, &sign_pub_key, &member_credential,
                                    member_precomp);
      if (kEpidNoErr != sts) {
        break;
      }
    ...
    if (signed_sig_rl) {
      ...
      sign_sig_rl = calloc(1, sign_sig_rl_size);
      if (!sign_sig_rl) {
    ...
    *sig_len = EpidGetSigSize(sign_sig_rl);
...
//#endif
    ...
    //  sts = EpidMemberSetSigRl(member, sign_sig_rl, sign_sig_rl_size);
    //  if (kEpidNoErr != sts) {
    //    break;
    //  }
    //}  // if (signed_sig_rl)
